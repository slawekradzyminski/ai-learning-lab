...
export function BackpropagationLabPage() {
  ...
  return (
    <div data-testid="backpropagation-lab-page">
      ...
      <section className="mt-6 grid gap-4 md:grid-cols-2"><Link to="/learn/how-llm-works/slides?slide=39" className="rounded-[2rem] border border-stone-200 bg-white/85 p-5 text-sm font-semibold text-sky-700" data-testid="backprop-slides-link">← Return to the instructor deck</Link><Link to="/learn/depth" className="group flex items-center justify-between rounded-[2rem] bg-slate-950 p-5 text-white" data-testid="backprop-depth-link"><span><span className="block text-xs uppercase tracking-[0.18em] text-sky-300">Next practical</span><span className="mt-1 block font-semibold">Use a hidden layer to solve XOR</span></span><ArrowRight className="h-5 w-5 transition group-hover:translate-x-1" /></Link></section>
    </div>
  );
}
