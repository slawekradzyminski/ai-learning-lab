...
export function DepthLabPage() {
  ...
  return (
    <div data-testid="depth-lab-page">
      ...
      <section className="mt-6 grid gap-4 md:grid-cols-2"><Link to="/learn/how-llm-works/slides?slide=43" className="rounded-[2rem] border border-stone-200 bg-white/85 p-5 text-sm font-semibold text-sky-700" data-testid="depth-slides-link">← Return to the instructor deck</Link><Link to="/learn/convolution" className="group flex items-center justify-between rounded-[2rem] bg-slate-950 p-5 text-white" data-testid="depth-convolution-link"><span><span className="block text-xs uppercase tracking-[0.18em] text-sky-300">Continue the neural track</span><span className="mt-1 block font-semibold">Compose learned spatial features</span></span><ArrowRight className="h-5 w-5 transition group-hover:translate-x-1" /></Link></section>
    </div>
  );
}
