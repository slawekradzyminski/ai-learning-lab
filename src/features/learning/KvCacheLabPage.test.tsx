...
describe('KvCacheLabPage', () => {
  test('recalculates memory and reports invalid head geometry', () => {
    ...
    expect(screen.getByTestId('kv-cache-attention-link')).toHaveAttribute('href', '/learn/attention');
    ...
    fireEvent.click(screen.getByTestId('kv-cache-reuse-choice-reuse'));
    fireEvent.click(screen.getByTestId('kv-cache-reuse-check'));
    expect(screen.getByTestId('kv-cache-reuse-feedback')).toHaveTextContent('Correct');
  });
});
