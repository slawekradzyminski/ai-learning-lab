    ...
    <div data-testid="kv-cache-lab-page">
      ...
      <section className="mb-6 flex flex-col gap-4 rounded-[1.5rem] border border-stone-200 bg-white/82 p-4 sm:flex-row sm:items-center sm:justify-between" data-testid="kv-cache-attention-bridge">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-slate-950">Start with the attention calculation</p>
            <p className="mt-1 text-sm leading-6 text-slate-600">When a new token arrives, earlier keys and values are unchanged and reusable. Only the new position needs a query to read from that history.</p>
          </div>
        </div>
        <Link to="/learn/attention" className="shrink-0 text-sm font-semibold text-sky-700 hover:text-sky-900" data-testid="kv-cache-attention-link">Review Q, K, and V →</Link>
      </section>
      ...
      <div className="mt-6">
        <LearningCheckpoint
          ...
          choices={[
            { value: 'smaller', label: 'Queries are always smaller vectors' },
            { value: 'reuse', label: 'The new query reads reusable earlier keys and values' },
            { value: 'accuracy', label: 'Caching queries would reduce model accuracy' },
          ]}
          correctValue="reuse"
