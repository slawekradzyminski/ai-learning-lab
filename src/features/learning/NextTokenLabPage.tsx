...
export function NextTokenLabPage({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <div data-testid="next-token-lab-page">
      ...
      <section className="mb-6 flex flex-col gap-4 rounded-[1.5rem] border border-stone-200 bg-white/82 p-4 sm:flex-row sm:items-center sm:justify-between" data-testid="next-token-attention-bridge">
        <div className="flex items-start gap-3">
          <Network className="mt-0.5 h-5 w-5 shrink-0 text-sky-700" />
          <div>
            <p className="text-sm font-semibold text-slate-950">Transformer blocks created the representation used here</p>
            <p className="mt-1 text-sm leading-6 text-slate-600">After many attention and MLP blocks update the residual stream, an output projection creates vocabulary logits and softmax turns them into probabilities.</p>
          </div>
        </div>
        <Link to="/learn/attention" className="shrink-0 text-sm font-semibold text-sky-700 hover:text-sky-900" data-testid="next-token-attention-link">Review attention →</Link>
      </section>
