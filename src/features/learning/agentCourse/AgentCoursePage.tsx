    ...
    <div className="space-y-8 pb-10" data-testid="agent-course-page">
      <header className="learning-enter border-b border-stone-300 pb-10 pt-2">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <p className="text-xs font-semibold text-slate-500" data-testid="agent-course-progress"><span className="font-mono text-amber-700">{completed.size}/{AGENT_COURSE_LESSONS.length}</span> complete</p>
        </div>
      ...
      <AgentCoursePipeline lesson={lesson} completed={completed} />
      ...
      <footer className="border-t border-stone-300 pt-8">
        ...
        <div className="mt-7 flex justify-center"><button type="button" onClick={() => { clearAgentCourseProgress(); setCompletedIds([]); }} className="inline-flex min-h-10 items-center gap-2 px-3 text-xs font-semibold text-slate-400 hover:text-slate-700"><RotateCcw className="h-3.5 w-3.5" /> Reset agent-course progress</button></div>
      </footer>
    </div>
  );
}
